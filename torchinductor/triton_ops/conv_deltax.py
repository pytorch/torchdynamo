...
from .conv_perf_model import early_config_prune, estimate_conv_deltax_time
...
@triton.autotune(
    ...
    prune_configs_by={
        ...
        'perf_model': estimate_conv_deltax_time,
        'top_k': 10
    },
)
...
class _conv_deltax(torch.autograd.Function):
    ...
    def _delta_x_ptr(wc, wh, ww,
                     shape_w,
                     dilation_h, dilation_w,
                     stride_xh, stride_xw, stride_xc, BLOCK_K):
    ...
    def _call(x, w, bias,
        ...
        if True:
            ...
            BLOCK_K = 32
            delta_x = \
                _conv_deltax._delta_x_ptr(wc, wh, ww, shape_w,
                                   dilation[0], dilation[1],
                                   stride_x[xh], stride_x[xw], stride_x[xc], BLOCK_K)
            ...
            # launch kernel, 1-dim
            grid = lambda META: (
                triton.cdiv(BATCH * OUT_H * OUT_W, META['BLOCK_M']) * triton.cdiv(KERNEL_N, META['BLOCK_N']),
            )
    ...
    def forward(ctx, x, w, bias,
        ...
        return _conv_deltax._call(x, w, bias,
                           stride, padding,
                           dilation, transposed,
                           output_padding, groups,
                           layout_x, layout_w, layout_y)
