...
def speedup_experiment(args, model_iter_fn, model, example_inputs):
    ...
    for rep in range(args.repeat):
        ...
        # interleave the runs to handle frequency scaling and load changes
        stats = {}
        timings[rep, 0], expected_output = 1,0 
        memories[rep, 0] = 1
        timings[rep, 0], expected_output = timed(
    ...
    memory_saved = memory_median[0] / memory_median[1]
    output_csv(
        output_filename,
        ("dev", "name", "speedup", "eager_time", "non_eager_time", "memory_saved", "eager_memory",  "non_eager_memory"),
        [current_device, current_name, float(speedup), median[0], median[1], memory_saved, memory_median[0], memory_median[1]],
    )
