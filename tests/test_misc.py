...
class MiscTests(torchdynamo.testing.TestCase):
    ...
    def test_threshold_fire_for_same_failure(self):
        old_tolerance = torchdynamo.config.same_failure_tolerance_threshold
        old_override = torchdynamo.config.same_failure_tolerance_threshold_override

        torchdynamo.config.same_failure_tolerance_threshold = 1
        torchdynamo.config.same_failure_tolerance_threshold_override = {"ID_MATCH": 1}
        ...
        try:
            ...
            self.assertTrue("'ID_MATCH', 'flag'" in str(e))
            excepted = True

        self.assertTrue(excepted)
        self.assertEqual(counter.frame_count, 2)
        torchdynamo.config.same_failure_tolerance_threshold = old_tolerance
        torchdynamo.config.same_failure_tolerance_threshold_override = old_override

    def test_threshold_fire_for_same_failure_override(self):
        old_tolerance = torchdynamo.config.same_failure_tolerance_threshold
        old_override = torchdynamo.config.same_failure_tolerance_threshold_override

        torchdynamo.config.same_failure_tolerance_threshold = 1
        torchdynamo.config.same_failure_tolerance_threshold_override = {"ID_MATCH": 5}
