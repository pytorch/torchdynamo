...
def same(a, b, cos_similarity=False):
    ...
    if isinstance(a, (list, tuple, torch.nn.ParameterList, torch.Size)):
        ...
        return len(a) == len(b) and all(same(ai, bi, cos_similarity) for ai, bi in zip(a, b))
        ...
        return all(same(getattr(a, key), getattr(b, key), cos_similarity) for key in a.__dict__.keys())
    else:
