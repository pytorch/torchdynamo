...
def clone_input(x, is_train=False):
    """copy while preserving strides"""

    def set_tensor_attributes(
        new_tensor: torch.Tensor, tensor: torch.Tensor, is_train: bool
    ):
        # Some tensor attributes are necessary to determine the outputs in custom
        # compiler. For example, if the requires_grad is not stored correctly,
        # the output generated by custom compiler can't call backward(...) and
        # training script is broken.
        # If someone found such attributes in the future, extend this function
        # to handle them.
        if is_train:
    ...
    with torch.no_grad():
        ...
        set_tensor_attributes(result, x, is_train)
        try:
...
def clone_inputs(example_inputs, is_train: bool = False):
    if isinstance(example_inputs, dict):
        res = dict(example_inputs)
        for key, value in res.items():
            assert isinstance(value, torch.Tensor)
            res[key] = clone_input(value, is_train)
        return res

    res = list(example_inputs)
    for i in range(len(res)):
        if isinstance(res[i], torch.Tensor):
            res[i] = clone_input(res[i], is_train)
    return res
